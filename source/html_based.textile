...
HTML-based apps use a subset of the entire SproutCore desktop library. When you generate an HTML-based app, you still get all of the core features of SproutCore, like property observation and binding. However, some of the UI niceties of the Desktop framework, like panels and most views, are not included. The Datastore framework is also not included. This greatly lowers the final size of your application. If, for instance, you would like to use the Datastore framework in your HTML-based app, you can include that framework in your +Buildfile+:

<shell>
config :all, :required => ["sproutcore/core_foundation", "sproutcore/datastore"], :theme => "sproutcore/empty_theme"
</shell>

NOTE: When generating a desktop app, you can use +SC.TemplateView+. HTML-based apps are a strict subset of the functionality available in desktop-based apps, so everything you see in this guide will work in a desktop app.
...
NOTE: At this point, you should download "the CSS":https://github.com/sproutcore/Todos-Example/raw/master/apps/todos/resources/stylesheets/todos.css and save it as +apps/todos/resources/stylesheets/todos.css+.
...
This property defines a binding on each of the item views. Each item will get the class +is-done+ if its associated content object is +isDone+. SproutCore will automatically dasherize property names into class names.
...
* May 5, 2011: added paragraph about the differences between HTML-based apps and traditional SproutCore Desktop apps, by "Colin Campbell":credits.html#colin
