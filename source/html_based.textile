...
Now that we know what our data looks like, let's create a controller to manage it. Since we want to
...
NOTE: At this point, you should download "the CSS":https://github.com/sproutcore/Todos-Example/raw/master/apps/todos/resources/stylesheets/todos.css
and save it as +apps/todos/resources/stylesheets/todos.css+.
...
During the development process, +sc-server+ makes it easy to test your application. Just run
the following inside of your project directory:
...
The first thing we need to do is add a checkbox to each todo view. If we want to handle user
input, we need to assign a view to the checkbox. In this case, we want to be notified whenever
the value of the checkbox is changed by the user. Remember that we assign views using the
+#view+ helper. Here's what the todo list looks like after updating it in +todos.handlebars+:
...
+todos.handlebars+ and put this at the bottom:
...
handling. Open +todos.handlebars+ and add a button inside the HTML for +statsView+:
