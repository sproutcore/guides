...
Let me begin by saying that you will rarely have to deal with the Run Loop.
However, it is worth gaining an understanding of it to understand more about
how your app works.
...
have to manage the run loop yourself is if you have a callback from an external
library that is not managed by SproutCore. In this circumstance, you will
want to trigger a new run loop by wrapping the code in +SC.RunLoop.begin()+
...
WARNING: Remember, you rarely need to manage the run loop manually. If you aren't certain of why you are using it, then you are probably doing something wrong.
