...
handle similar objects. In the above example, we created a person. Normally, however, it would make sense to create a +Person+ class, and then to create objects from it. In order to do this, we make a subclass of SC.Object
...
function. +sc_super+ is functionally very similar to the 'super' method in, for instance, Ruby.
...
Whenever an instance of +SC.Object+ is created, the +init+ method is called. This function can be overriden when you need to perform some setup functionality when creating your object.
...
NOTE: As you may have guessed, +SC.Object.extend+ just takes a series of
...
properties. In this case you can use the mixin method. Note, that the mixin method is different from the 'mixins' described above. The mixin method will add the properties and methods within the block as class properties and methods. Mixins themselves add instance variables and methods.
...
arguments to the property function. The +get+ function will call a computed property with this +value+ set to +undefined+, where the +set+ function will set +value+ to the property given in the parameters to set.
...
NOTE: In most cases you will find you can just ignore the +keyName+ parameter.
...
location. As you can see, the relative property begins with a period. This will
...
updated the function is called. You can also pass multiple properties into an observer.
...
changes. The +set+ method handles this for us automatically - this is one of the reasons +get+ and +set+ should be used to access properties. However, there are cases where using
...
once +endPropertyChanges+ is called and will prevent unnecessary duplicate
notifications.
...
The asterisk (+*+) in the property path
indicates that we want SproutCore to observe changes to all properties
following the asterisk. In this case, that is both +mainUser+ and +name+.
...
which isn't necessary. It is advisable in practice to use chained observers as little as possible, in order to protect performance.
...
NOTE: One very important difference between bindings and observers is that observers updated immediately, where bindings update only at the end of each run loop. This offers considerable improvements in performance, but can be confusing if you are experimenting at the console, since in that setting the run loop must be triggered manually. See 'The Run Loop' for details on how to do this.

In this example, we used the absolute path to the property. Bindings
also support relative properties and chained properties as well.
...
bindings. By default, if you only specify a string for the binding it is created
as if you had done +SC.Binding.from(STRING)+.
...
You will rarely have to reference the Run Loop
...
* March 1, 2011: fixed grammar, clarified phrasing, added examples to the Observer section by Jason Gignac
