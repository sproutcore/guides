...
* Write your own +Enumerable+ object
...
For instance, all <tt>Enumerable</tt>s support the standard +forEach+ method:
...
In general, SproutCore objects that represent lists implement the +Enumerable+ interface. When working with views that expect to work with a list of things, like a CollectionView or TemplateCollectionView, this means that any list-like construct will work. Some examples:
...
* +RecordArray+: An Array-like object that represents a list of Record objects. <tt>RecordArray</tt>s are "live", and the Data Store can update them when new data comes in from the backend.
* +Set+: An object that can quickly answer whether it includes an object.
* +jQuery+: SproutCore extends the jQuery object with +Enumerable+
...
You can get the first or last object from an Enumerable by +get+ing +firstObject+ or +lastObject+.
