...
Now implement +Todos.MarkDoneView+, which we just referenced. Since our view implements a checkbox, we will subclass the +SC.Checkbox+ control. This gives the view a +value+ property that reflects the value in the DOM, as well as a +title+ property that displays a label for the checkbox.
