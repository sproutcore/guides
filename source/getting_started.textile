...
You will benefit from carefully following the instructions in this guide, to create directories, to type in code (or to copy and paste it), because it will register in your mind more solidly. However, you can access the finished source code for this application on "Github":https://github.com/sproutcore/Todos-Example/. To see it in action, click "here":http://github/sproutcore/sproutcore. [TODO] need a demo app url [ATTN: tce]
...
We can describe these intelligent aspects, generally, as the "system of connections" or, in the manner an electrician would use, as the "wiring diagram." SproutCore apps have _bindings_ between properties of various components of the user interface and those of underlying machinery. You see the components of the user interface: checkboxes, buttons, text fields, list views, etc. We'll be looking at the underlying machinery. SproutCore apps have another type of intelligence: they have _states_ that divide the action. We'll cover _bindings_ and _states_ in more detail. Register these words as centrally important.
...
* *learning_sc/*
** *Buildfile*             | _Configuration for apps_
...
***** *loading_app.js*     | _The LOADING_APP state, where data is loaded, and the UI configured_
...
You see that SproutCore is a powerful system, challenging to master. We'll take it one step at a time.
...
h4. The App Root Directory and its Files
...
There are four states in the Todos statechart: STARTING, LOGGING_IN, LOADING_APP, and SHOWING_APP. These are defined in individual files in the states directory.
...
|_. starting.js |_. logging_in.js |_. loading_app.js |_. showing_app.js |
...
When the statechart is initialized from Todos.main(), program control will go to the STARTING state, specifically to its enterState() function, where there is a check for the storeType. storeType was set to be 'FromFixtures' in core.js, where the data store is instantiated. (Recall that using fixtures data as the data store is a way to simulate a connection to a real database). You see in the if-else block of STARTING/enterState() that, because storeType === 'FromFixtures', the gotoState() call moves control directly to the LOADING_APP state, not the LOGGING_IN state, which we won't use in the Todos app. 

There are different conventions for naming states. The style used here is proven, and the formatting makes the state names prominent. Some people like to camel-case the state names names.
...
We won't use the LOGGING_IN state in Todos, but have it here for illustration.

h5. LOADING_APP State

<javascript filename="in apps/todos/states/loading_app.js">
// ==========================================================================
// Project:   Todos
// Copyright: @201X My Company, Inc.
// ==========================================================================
...
Find the LOADING_APP/enterState() function, where the comments mark three steps:

* *Step 1*: Append the mainPage view, which we'll be adding next. Appending a view effectively causes the view to be shown.
* *Step 2*: Set the content property of the todosController, an SC.ArrayController that holds the main todos data. Recall that the store was initialized from fixtures. We are now loading data from that store and setting it into the todos array controller. In a SproutCore app, the wiring diagram provided by javascript code to set up bindings between views and controllers and the statechart makes the action happen. Simply by setting content of the todosController, because it is wired to the todo item list view by a binding, the initial todos data loaded from the store (and behind that, from fixtures) will show automatically. If this Todos app were to be used in a real-world setting, you would simply need to set the store up to pull from a REST datasource on the web instead of from static fixtures data.
* *Step 3*: Place keyboard focus on the new todos item text field, so there is a blinking cursor cueing the user to type a todo.
...
The SHOWING_APP state is equivalent to the "ready" state created by sc-gen for a Hello World app. This state handles two actions, addTodo and clearCompletedTodos. There are references to parts of the view code that will become clear after we add the resources directory, with the mainPage view and associated files, and other view and system components. Look back to the statechart code as we move through the rest of the guide.
...
The underscore starting the name of this file is a dead giveaway that it must apply to the theming system. It defines $theme to be our Ace-derived theme, ace.todos. You'll see $theme used in individual content CSS files, so now you know where it is defined.
...
CSS is an art unto itself, an important side of doing SproutCore development and building views.
...
totalTodosBinding is simple enough. It is a one-way binding (we are "pulling", we don't care about pushing back) to the length of the todosController array controller. We need a local reference to this length, because we are about to do something slick, in part based on it.
...
areAllCompleted() is a function, but a very special type of function, called a computed property. Note the .property() call hanging on the end of the function definition. This gives 'length' and 'totalTodos' as parameters, then tacks on .cacheable(). First, the parameters, which are references to local properties for this context, which is the array controller body. We know we have a 'length' property -- this is an array controller. But what gives with the use of the 'totalTodos' reference, isn't the property called totalTodosBinding? Ah, the answer is that anytime Binding is appended to a property name, you have a kind of dual-property arrangement. The actual property is, in fact, totalTodos. In defining totalTodosBinding, we essentially say, get the value of totalTodos from this other source. That covers the background on mechanics of property names, now to address what .property('length', 'totalTodos').cacheable() accomplishes. It means:
  ...
  "Base this computed property on the length and totalTodos properties, and update it if either of them change, and, oh, by the way, please cache the values while you are at it, so unnecessary updates aren't performed." 

Wow! That was a lot to grok, but think of the power here! It really is slick. 
...
So, a computed property used in this way is both a setter and a getter. Complicated, but useful. Once you get the hang of making these you realize the benefit: the automatic updating gain is worth the learning pain.
...
* February 27, 2012: rewrite for SproutCore 1.8 by the 1.8 release sprint team, including the following who worked on this task: "Tim Evans":credits.html#tce, "Jeff Pittman":credits.html#geojeff, and others [TODO] Add 'em!
