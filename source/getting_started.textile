...
Download the SproutCore Starter Kit if you haven't already. Unzip and it open the directory in your favorite text editor.
...
NOTE: We've provided a simple stylesheet to give the application some style.  At this point, you should download the "CSS":https://github.com/sproutcore/Todos-Example/raw/master/apps/todos/resources/stylesheets/todos.css, add it to your project, then include it in your page:
...
To start building our views, let's open +index.html+.  You'll see that the starter kit includes a default Handlebars template. Remove it and replace it with the following:
...
Let's also add a header right above the template:

<html>
<h1>Todos</h1>
</html>

Finally, add a unique +id+ attribute to the +body+ tag so we can style with CSS:
...
<html filename="index.html">
{{#collection contentBinding="Todos.todoListController" tagName="ul"}}
  {{content.title}}
{{/collection}}
</html>
...
Handlebars expressions, like +{{remainingString}}+, allows us to automatically update the DOM when a property on our view changes. In this case, we're saying that the content of the <code>Todos.StatsView</code> should be bound to the value of the view's +remainingString+ property.  As with the other bindings, this will be updated for us automatically whenever the value changes.
...
We say that +remainingString+ *depends on* <code>remaining</code> because it requires another value to generate its own value. We list these *dependent keys* inside the +property()+ declaration.

We've also bound the view's +remaining+ property to the <code>todoListController</code>'s +remaining+ property. This means that, because of the dependency chain we've described, if <code>Todos.todoListController.remaining</code> changes, +remainingString+ will automatically update.
