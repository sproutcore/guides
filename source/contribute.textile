...
The easiest way to get started with the guides on OSX is to download and install the "guides pkg":http://guides-pkg.strobeapp.com/Guides-0.6.6.pkg. This will give you the +guides+ executable on your machine, and does not require you to have a C compiler on your computer.
...
For the rest of this guide, when the +guides+ command is referenced, prefix it with +bin+. For instance, if you are instructed to run <tt>guides preview</tt>, instead run <tt>bin/guides preview</tt>.  Or, you can add the +bin+ subdirectory to your *path* so you don't have to remember to do this.
...
* Guides are written in a modified Textile. For example, take a look at existing guides, or some of the "Rails Guides":https://github.com/rails/rails/tree/master/railties/guides/source for some examples
