...
You have learned in previous guides how to use **sproutcore gen** to make a project directory and to create apps in the project apps directory. You have learned how to run sc-server in the project directory, and how to view apps in a local browser at localhost:4020/. You have learned about SproutCore app concepts, including the use of a buildfile and of views and a statechart. We will build on that foundation to produce a TodosThree app with components of a best-practices SproutCore app.
...
[TODO] **sproutcore gen** for statechart creates a main.js with:  SC.RootResponder.responder.set('defaultResponder', statechart); -- do we need this?

main.js for TodosThree is very simple. The statechart is set as the default responder, so it gets events. Then the statechart, the brains of the outfit, is initialized.
...
**sproutcore gen** created a file called ready_state.js in the states directory. We aren't going to use it for the TodosThree app, because we'll need several states.
...
The footer view has a single child view, clearCompletedTodos, which is a button whose action is tied to the clearCompletedTodos() function in the completedTodosController. The isEnabledBinding property has a one-way binding (faster, if you don't need two-way, reciprocal updating) to the length of the completedTodosController. If its length is 0, then isEnabled is false and the button will not be active (If there aren't any todos, there is nothing to clear).
...
The todosList view follows the SproutCore idiom for wrapping an SC.ListView inside an SC.ScrollView. The contentBinding property of the list is set to TodosThree.todosController. The exampleView property introduces a concept very important for SproutCore: views can be defined in separate javascript files, often held in a views directory, and included where they are needed. exampleView is set to TodosThree.TodoItemView, which necessitates the require statement at the top of the file: require('views/todo_item');. TodoItemView has a checkbox and label. We'll add it when we add the views directory later.

INFO: The use of require('views/todo_item'); forces a load order. You know that after building, the app will consist of all these various files pulled together, but in some cases you need to insure that contents of one file come before another. The directory hierarchy doesn't matter so much as the file names: the default load order is alphabetical by file name, unless modified by such require statements. You may see a console error to indicate a problem related to a dependency, and you will need to fix by adding such require statements.
...
TodoItemView is a checkbox, which has a label. You see a bit of wiring to CSS in the classNames property, an array of CSS class names that apply to this view. valueBinding has a reference to .content.isCompleted, prompting an explanation of the context for this view. We know that this view is defined as the exampleView property of the todos list. The term example here means "the view to use for each list item." List items for the todos list are todos, which are defined in the TodosThree.Todo model. So, .content used in the TodoItemView definition refers to a single todo item in the list, so .content.isCompleted refers to this property of a todo item. Likewise, titleBinding is tied to the title property of a todo item.
...
NOTE: Recall discussion of good habits regarding require. The use of require('core'); here is not mandatory, but it helps as a reminder of the loading order when abbot builds your application.
...
**sproutcore gen** does not make an en directory, nor its fixtures directory. We'll make these.
...
<shell>
$ pwd 
    ~/development/sproutcore/getting_started/apps/todos_three/en/
$ mkdir fixtures
$ cd fixtures
</shell>
...
<shell>
$ pwd 
    ~/development/sproutcore/getting_started/apps/todos_three/en/fixtures
$ cd ../../../..
</shell>
