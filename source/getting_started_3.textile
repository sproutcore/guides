...
some task, fleshed out with needed properties, bindings, workhorse functions and the like. _panel is a such
local property on the state, marked with a beginning underscore to signify that it is not connected 
to anything external. 

We have a local reference to the completedTodosController, which is used with the javascript map() function
to produce an html-coded list of completed todos, added to the displayDescription string for the alert pane. 

The alert pane has its defaultResponder set to the statechart. Recall that when you see this, you can scan the
code for actions that will fire to the statechart. We see them in the button definitions for the alert pane,
set as a convenient array of objects containing the title and action of each button.

The confirm() function is the business end of the delete, from which we go back to the SHOWING_APP state when
done with the destroy call. If the user clicks cancel, instead, we simply go back to the SHOWING_APP state.

For a real view that pops up (not an alert, which is special), the view would be taken down in exitState(). 
...
To set up a binding, we tack Binding onto the end of a property declaration, as with xBinding = something, in which
case we would be creating a property called x, which works via a binding. A binding is a connection between properties,
which could be a single value, an array, or other basic object. Whatever it is, when the property's value is needed, it
will be grabbed from the defined "other side." For the "side" terminology, we can distinguish between the "to" side and
the "from" side. The default binding is a "two-way" binding, in which either side is updated when the other side changes.
...
that is referred to as totalTodos. You see this in the two this.get() calls where the completedTodos are 
subtracted from the totalTodos. In SproutCore, this also exemplifies the way to .set('propertyName') and 
.get('propertyName') values. 

NOTE: Computed properties, so important in SproutCore, are given a fuller description below.
...
the scope here, of the entire file, and within that, he page. 
...
built-in property of an array controller.
