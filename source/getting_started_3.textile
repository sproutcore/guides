...
INFO: Here we are creating an observer in a state, so we use the dedicated .stateObserves() observer style,
but in general use, the observer function, used in exactly the same way, is simply called .observes(). 

INFO: There are nuances to learn about specifying paths in observer declarations, such as how to use an asterisk 
