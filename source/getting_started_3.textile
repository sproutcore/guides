...
When the statechart is initialized in the TodosThree.main() function, program control will go to the STARTING state, specifically to its enterState() function, where there is a check for the storeType. storeType was set as 'FromFixtures' in core.js, where the data store is instantiated. You see in the if-else block of STARTING/enterState() that, because storeType === 'FromFixtures', the gotoState() call moves control directly to the LOADING_DATA state, not the LOGGING_IN state, which we won't use in the TodosThree app. 
