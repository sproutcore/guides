...
NOTE: Learning how to use a distributed versioning system such as git and and the related github is valuable. 
You can create your own free membership if you don't already have one (or use an internal company system). If
you make your own repo to which you do commits as you follow along here, you should make it a private repo if you 
don't have a compelling reason to share it, and if it isn't modified to be any different from the one here. 
Where you see *GIT* messages interspersed on this page, you may issue the commands for your own repo if you wish.
...
INFO: This is a common pattern in SproutCore apps: Use bindings whenever you can to solve most problems, but 
...
NOTE: Here we are creating an observer in a state, so we use the dedicated .stateObserves() observer style,
...
The isDefaultBinding property also involves an interesting path. You note an asterisk in front of a reference to
...
NOTE: The sc_require statements at the top of each of these files illustrate a concept that you learned in a
previous guide: views can be defined in separate javascript files and included where they are needed. The load 
order for javascript files will be alphabetical, unless otherwise controlled with sc_require().  Sometimes you 
