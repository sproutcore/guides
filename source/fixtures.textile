...
SproutCore looks for fixtures assigned to +MyApp.MyModel.FIXTURES+. By convention, fixtures are defined in +my_app/fixtures/my_model.js+. Fixtures are not included in production builds. In order to include them in production builds, you can add it via your +Buildfile+:

<ruby filename="Buildfile">
config my_app, :load_fixtures => true
</ruby>
...
It is also possible to specify attributes that are not explicity defined in your model, although explicit declaration is encouraged.
...
<javascript filename="apps/my_app/fixtures/company.js">
...
When the fixtures are loaded into your app, the relationships are automatically computed and point to the appropriate records.
...
This simulates a remote response with 500 milliseconds round-trip latency.  Tweak the value to match the performance of your network and servers. Simulating latency is important as your application will eventually be communicating with a persistent datastore that has inherent latency. It ensures your application code is written in an asynchronous manner and that is will work when latency is involved.
...
TIP:  Notice the quotes around the data source name?  +MyApp.MyAppsFixturesDataSource+ doesn't exist until its instantiated during the loading process. SproutCore will automatically find your data source after its been loaded.
...
* May 5, 2011: Minor tweaks and additions, by "Colin Campbell":credits.html#colin