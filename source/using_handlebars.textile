...
NOTE: You can add an attribute data-view to your +<script>+ tag to link it to a view.

To make a template available to be used later, give the +<script>+ tag a data-template-name attribute and put it in the +<head>+ tag:
  
<html>
  <head>
    <script type="text/html" data-template-name="say-hello">
      Hello, <b>{{MyApp.name}}</b>
    </script>
  </head>
</html>
