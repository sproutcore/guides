...
This guide will walk you through generating documentation for your sproutcore classes in a format that is friendly to jsdoc-toolkit, which is the library SC uses for generating docs from the code.

endprologue.

h3. Overview

CoreWeb uses the jsdoc-toolkit (http://code.google.com/p/jsdoc-toolkit/) to generate its documentation. As such, your CoreWeb classes should follow strict code-documenting formatting guidelines in order to properly generate your HTML documentation. This document outlines the proper  format you should use.
