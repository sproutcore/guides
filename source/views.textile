...
Let's make the interface a bit more visually appealing and user friendly by giving a usage hint when no contact has been selected yet. We can create a generic view and add the class name +myapp-usage-hint+ to the +classNames+ array, and then a label view as a child to display the hint. Our +bottomRightView+ should now look like this:
