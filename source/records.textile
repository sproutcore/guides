...
NOTE: The model layer is also responsible for fetching data from and commiting changes to your server when necessary. The server communication aspect of the model layer is not covered in this guide, but in the guide about using data sources.

Models define the data schema of your application. In SproutCore, models are defined by subclassing +SC.Record+. When you actually want to create a data record from one of your models, you use +SC.Store+ to create an instance of an +SC.Record+ class. The store manages the lifecycle and the data of your records in a central place. When you retrieve or update a property from a record, the record actually uses the store to access the underlying data hash.

NOTE: The classes that SproutCore's model layer rely on are located in the "datastore" folder inside of the sproutcore source code. The code has plenty of inline documentation and can be a valuable resource to gain deeper insights in how the store works.
...
Beneath the hood, a SproutCore record consists of four main components:
# a store key
# an id
# a status
# a data hash
...
All records of a certain type have a unique id as usual in relational database systems. In fact the ids of SproutCore records usually are the same as the primary keys of your data in the backend. Unlike the store key, the id is not automatically created. It is your responsibility to assign a unique id to a record when the record is created.

The status of a record represents its current state with respect to the corresponding record on the server. The store uses the status property to determine if a record can be edited safely, and to see which records need to be committed back to the server. 
...
The names of these states are pretty self explanatory: EMPTY indicates that the record is not yet created. READY indicates that the record can be safely edited. BUSY indicates that the record is currently locked for write operations, usually because it is in the middle of communicating with the server. DESTROYED indicates that the record has been destroyed. ERROR indicates that something went wrong while processing the record.

NOTE: The three main states READY, BUSY and DESTROYED, each have several sub-states. We will learn about these as we work with records, below. A summary  of all the states and sub-states is located at the end of this document.
...
We have used the +SC.Record.attr+ helper to add the +firstName+, +lastName+ and +age+ attributes with the type of the attribtue as first argument. +SC.Record.attr+ also accepts an option hash as an optional second argument. This has can, for example, allow us to add some default values to our attribtues:
...
WARNING: The +defaultValue+ will not be written to the underlying data hash and therefore not committed back to the server. It will be returned by your Sproutcore application if the value in the data hash is +null+ or +undefined+.

If the name of the model's attribute property differs from the name used in the data hash, you can specify a custom key for each attribute which will be used to access the data hash. This lets you map your names to, for instance, less sane or syntactically inappropriate names that you may have received from an external data source:
...
All of the basic JavaScript data types can be used as attribute types: 
* String
* Number
* Boolean
* Array
...
NOTE: To learn more about how to specify your SC.DateTime format, refer to the documentation on the  "SC.DateTime#toFormatedString":http://docs.sproutcore.com/symbols/SC.DateTime.html#SC.DateTime%23toFormattedString method.
...
WARNING: Under normal circumstances, you should never change the id of an existing record using +set()+. This can cause a number of complex issues.

NOTE: It is a best practice to never include the id in the data hash, because then you end up with two ids: the id property in the data hash and the id managed by the store. If you receive a JSON data hash from the server (where the id is necessarily included) then you should extract and delete the id from this hash before using it to load the record into the store.


h4. Relations

Often, models are related to other models. For example, the +Contact+ model we created above may be related to one or more instances of an +Address+ model. Sproutcore allows you to define this relationship as part of the model definition.
...
The two most common options for relationships are 'isMaster' and 'inverse'.

The +isMaster+ option controls when related records are marked as changed. The "+isMaster: YES+" option on the +address+ attribute makes sure, that the +Contact+ record gets marked as changed when you assign a different +Address+ record to it. 

NOTE: Generally, you should set the +isMaster+ option to +YES+ on one side of the relation and to +NO+ on the other to control which record is committed back to the server when the relation changes. 
...
NOTE: In the underlying data hash, a +toOne+ relation is simply represented as the id of the associated record.
...
NOTE: In the underlying data hash, a +toMany+ relation is represented as an array of ids of the the associated records.

h5. Many-to-Many Relations

If we not only want to relate multiple addresses to one contact, but also relate multiple contacts to one address, we have to use +toMany+ on both sides of the relation. SproutCore's +toMany+ helper manages many-to-many relations without the 'join table' you would use in a relational database:
...
NOTE: A many-to-many relation is constructed by using +toMany+ on both sides. Thus, it is represented in the underlying data hash as an array of record ids on both sides of the relationship.


h4. Other Properties on Model Classes

Any property defined on a model class not using +SC.Record.attr+ is a transient property. This means that its value is not passed through to the data hash of the record, and is neither committed back to nor loaded from the external data source.
...
WARNING: If you use +set()+ on an undefined property, SproutCore by default will pass the value through to the underlying data hash. You can turn this behavior off by setting +ignoreUnknownProperties: YES+ in your model classes.
...
NOTE: In the example above we are using fixtures as the data source. You can read more about fixtures and using other data sources in the respective guides.
...
The first argument of the store's +createRecord+ method is the record type. The second argument is a hash with optional initial values. You can specify the record id as third argument:
...
NOTE: Usually, you will not specify an id like this, because the id will generally be supplied from an external data source.

NOTE: Ids can be numbers or strings.
