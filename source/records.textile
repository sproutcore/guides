...
The default value will be returned whenever the attribute's actual value is +null+ or +undefined+.

NOTE: The default value will not be written to the underlying data hash and therefore not committed back to the server. 
...
For a reference of how to specify your custom date format check the documentation of  "SC.DateTime#toFormatedString":http://docs.sproutcore.com/symbols/SC.DateTime.html#SC.DateTime%23toFormattedString.
...
Now that we have defined our +Contact+ and +Address+ models it's time to actually create some records. All records are managed by the store, so you have to make sure first that you have an instance of +SC.Store+ available. Usually the store is instantiated somewhere in your application's "main.js" file:
...
if ((status & SC.Record.READY_NEW) == SC.Record.READY_NEW) { 
  // ...
}
</javascript>
...
--* February 13, 2011: corrected error in query sorting
* February 9, 2011: typos and other minor corrections by Florian Kugler
