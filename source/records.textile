...
WARN: This works great if you are in control of your primary keys.  What do you do if you are relying on your datasource to set the key for you?  The answer is you must commit the master record first like the following.

<javascript>
	myContact = MyApp.store.createRecord(MyApp.Contact, {}); 
	myContact.commitRecord();
	myAddress = MyApp.store.createRecord(MyApp.Address, {});
	//now push the child to the parent
	myContact.get('address').pushObject(myAddress);
	//should these be committed? probably
	myContact.commitRecord();
	myAddress.commitRecord();  // I wonder if there's an auto-commit function?
</javascript>
h4. Updating Records
...
h3. Committing Records

How do the changes introduced in the local store propagate back to the main data source?  Committing your records calls the underlying datasource's methods.  The details are beyond the scope of this guide.  The general form is:

<javascript>
	var result = myRecord.commitRecord(); 	
</javascript>

WARN: Why do we assign the result of the method to a variable?  Because we want to check the status of the record.  result.status!
